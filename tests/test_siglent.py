...
def unloaded_configuration(
    ...
    *,
    ...
    return replace(
        ...
        acceptance_context="no_load_test",
    )
...
def test_no_load_test_outside_band_current_trips_and_uses_safe_order(
    tmp_path: Path,
    ...
    with pytest.raises(PowerControlError, match="safe band") as captured:
        control.set_current(target_current_a=0.4, expected_current_a=0.2)

    assert captured.value.uncertain_output is False
    trip = control.read_state().no_load_test_interlock.trip
...
def test_no_load_test_inclusive_safe_band_does_not_latch(
    tmp_path: Path,
    measured_current_a: float,
) -> None:
    device = FakeDevice()
    ...
    assert result.state.no_load_test_interlock.status == "unlatched"
...
def test_unavailable_measurement_on_write_free_replay_stops_process(
    tmp_path: Path,
    ...
    with pytest.raises(
        PowerControlError, match="unavailable or non-finite"
    ) as captured:
        control.set_current(target_current_a=0.4, expected_current_a=0.2)
...
def test_every_successful_no_load_test_mutation_queries_fresh_measured_current(
    tmp_path: Path,
    operation: str,
) -> None:
...
def test_no_load_test_read_diagnostic_is_write_free_and_does_not_create_latch(
    tmp_path: Path,
) -> None:
    device = FakeDevice()
    config = unloaded_configuration(tmp_path)
    ...
    assert state.no_load_test_interlock.status == "unlatched"
    assert state.no_load_test_interlock.trip is None
    assert device.events == ["read_identity", "read_state", "close"]


def test_process_trip_blocks_prepare_but_allows_shutdown_and_fresh_process(
    tmp_path: Path,
):
    device = FakeDevice()
    config = unloaded_configuration(tmp_path)
    control, factory = controller(tmp_path, device, config=config)
    device.measured_current_read_sequence = [0.002, 0.0]
    ...
    device.measured_current_read_sequence = [0.0]
    result = control.shutdown()
    assert result.state.output_enabled is False
    assert result.state.no_load_test_interlock.status == "latched"
    fresh, _ = controller(tmp_path, device, config=config)
    assert fresh.read_state().no_load_test_interlock.status == "unlatched"
    assert fresh.prepare().state.output_enabled is False
